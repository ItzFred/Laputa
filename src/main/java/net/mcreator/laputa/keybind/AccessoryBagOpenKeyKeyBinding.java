...
public class AccessoryBagOpenKeyKeyBinding extends LaputaModElements.ModElement {
	...
	public void initElements() {
		keys = new KeyBinding("key.mcreator.accessory_bag_open_key", GLFW.GLFW_KEY_I, "key.categories.inventory");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					...
					LaputaMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(1, dt));
